...
只碰七类东西：**codex ticket 相关的功能**、**OpenAI 账号消耗节奏**（旧版选号的重排，设计见
...
| **非门控模型只观测，不注入也不探测** | 三个准入入口（`PrepareUpstream` / `PrepareWSTurn` / `PrepareWSMapPayload`）对非门控模型返回**不带票的 attempt**：照记收发两侧的 state、票按它自己的模型名入库，但不注入、不拦截。**诊断探测要按门控集合挡住**（`maybeObserveProbe`）：新模型多半不在校准资料里，闭集归因会把票硬判成最像的已知模型，结论不成立、额度白烧。**握手票仍只对门控模型收**（`ShouldObserveHandshakeState`）：它挂在物理连接上只能消费一次，非门控轮次先收走会让同一连接上随后的门控轮次收不到 |
