...
只碰三类东西：**codex 票的被动观测**（收票与请求特征，设计见 `DESIGN-codex-ticket.md`）、
**OpenAI 账号指纹测试**（设计见 `DESIGN-openai-fingerprint-test.md`），以及**fork 自身必须适配的部分**（版本检查、
...
| **不要跑 `wire generate`** | codex 票的观测与指纹测试的装配是手工写在 `cmd/server/wire_gen.go` 里的（直接给 `AdminHandlers` 赋字段、调 `SetKongTicketObserver`，外加收票写入的 `Start` 与 cleanup 里的 `StopKongTicketObserver`），wire 生成不出这几行。重新生成会把它们抹掉，而编译不会报错——只会让功能静默失效 |
