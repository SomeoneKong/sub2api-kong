...
// 草稿在首次渲染时就按服务端行建好了，替换服务端行时要让它跟上：**用户没改过的字段**（仍等于旧服务端
// 值）换成新值，改过的保留。不跟的话，别处把模式从 full 改成 off 之后，这一行下拉仍显示 full，用户
// 只改个出口再保存，就把模式悄悄写回了 full。
function rebaseDraft(prev: TicketAccountStatus, next: TicketAccountStatus): void {
  const draft = drafts[next.account_id]
  if (!draft) return
  const proxyText = (row: TicketAccountStatus) => (row.proxy_id === null ? '' : String(row.proxy_id))
  if (draft.mode === prev.mode) draft.mode = next.mode
  if (draft.egress === prev.egress) draft.egress = next.egress
  if (draft.proxyIDText.trim() === proxyText(prev)) draft.proxyIDText = proxyText(next)
}
