<template>
  <AppLayout>
    <div class="mx-auto max-w-[1400px] pb-8">
      ...
      <!-- 按账号 key：同一路由换账号时组件实例会被复用，不重建的话事件表还停在上一个账号。 -->
      <TicketEventTable
        v-if="page?.account"
        :key="accountID"
        class="mt-6"
...
// 必须随路由变：同一路由只换参数时（例如从浏览器历史直接跳到另一个账号）Vue Router 复用组件实例，
// setup 不会重跑。只在 setup 里取一次，页面会继续查询、显示并验证上一个账号。
 ...
 * 事件筛选器的模型选项。
 *
 * **不能只从票推**：取票失败、`fetch_skipped`、入库失败都产生事件却不产生票，而票还会被过期清理、
...
// 只认最后一次发起的加载：换账号后，上一个账号迟到的响应不能覆盖新页面。
let loadSeq = 0
...
async function load(): Promise<void> {
  ...
  loadError.value = ''
  try {
    const next = await codexTicketAPI.getTicketDetail(accountID.value)
...
async function verify(t: TicketDetailRow): Promise<void> {
  ...
  try {
    const res = await codexTicketAPI.verifyTicket(id, t.id)
    // 验票期间换了账号：结论属于上一个账号，不能写到新页面上。
    ...
    if (res.detail) {
      ...
      // 验票已经完成，只是补读失败：列表还是旧的，不能让它不声不响地顶着旧结论。
      notes.value[t.id].text += '；状态刷新失败，列表可能仍是旧状态，请刷新页面'
    }
  } catch (error) {
    if (id !== accountID.value) return
...
function statusText(t: TicketDetailRow): string {
  if (liveRemaining(t) <= 0) return '已过期'
  switch (t.status) {
    case 'verified':
...
function statusClass(t: TicketDetailRow): string {
  if (liveRemaining(t) <= 0) return 'badge-gray'
  switch (t.status) {
    case 'verified':
