        ...
        <tbody class="divide-y divide-gray-100 dark:divide-dark-700">
          <template v-for="ev in events" :key="ev.id">
            <tr class="align-top">
              ...
              <td class="px-3 py-2 text-xs text-gray-500 dark:text-dark-400">
                <!-- 票 id 与用量明细的「注入票 #N / 上游回发 #N」同一个口径，凭它把两边对上号。 -->
                <span v-if="ev.ticket_id !== null" class="whitespace-nowrap">票 #{{ ev.ticket_id }} </span>
                    ...
                    <tr v-for="probe in probes" :key="probe.id">
                      <td class="py-1 pr-4">
                        ...
                        <span
                          ...
                          title="来自取票请求本身（融合取票）：产生于票据出口、当时没有注入票"
                        >融合</span>
                      </td>
                      ...
                      <td class="py-1 pr-4">{{ probeReasonText(probe) }}</td>
...
/**
 * 一份探测为什么没计入平均。作废（回答本身无效）与弃用（回答留档、只是产生条件不同而不采用）分开说：
 * 融合样本归因不达标时 invalid_reason 为空，只看它会显示成"没有原因却没计入"，还容易被读成票被判死。
 */
function probeReasonText(probe: FingerprintProbe): string {
  const parts: string[] = []
  if (probe.invalid_reason) parts.push(reasonText(probe.invalid_reason))
  if (probe.discarded_reason) parts.push(`未采用：${reasonText(probe.discarded_reason)}`)
