...
/**
 * 四条展示路径共用这张表：单份挑战的作废原因、融合样本的弃用原因、验证未得出结论的原因、取样的
 * 处置原因。键取自后端写进 detail 的 reason 与 probe.invalid_reason / probe.discarded_reason，缺键会
 * 原样显示英文串。
 *
...
const reasonTexts: Record<string, string> = {
  ...
  ticket_expired: '票已过期',
  stg0_model_mismatch: '上游回报的模型不符',
  // 融合样本弃用（探测明细里 discarded_reason 非空的那一份）：回答留档但不采用，票本身不受影响
  fused_unreadable: '融合取票正文未读回',
  fused_unparsable: '融合样本无法解析',
  fused_insufficient: '融合样本归因未达标',
  fused_stg0_mismatch: '融合样本的上游回报模型不符',
  // 验证未得出结论
