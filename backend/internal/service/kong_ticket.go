...
type KongTicketRepository interface {
	...
	// 那就该挑剩余 TTL 最长的。批量取票之后最新那张还通常正是刚在正常档窗口里取回的 fetch 票。
	...
	// VerifiedTicketsElsewhere 返回**别的账号**在同一模型上此刻可用的全部票，供调用方按当前接受白名单
	// 重判。不按账号截断：截断发生在重判之前，会把"新票不合格、旧票仍合格"的账号误判成接不了手。
	//
