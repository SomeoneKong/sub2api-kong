...
// kongZstdReadBody 经 GetBody 读出明文，不消耗 request.Body。ContentLength 此时就是明文长度，用它一次分配好
// 缓冲，免得 io.ReadAll 从小缓冲倍增扩容；它只作提示，不限制读取量。
func kongZstdReadBody(request *http.Request) ([]byte, error) {
	...
	if n := request.ContentLength; n > 0 && n <= kongZstdMaxPreallocBytes {
...
// kongZstdMaxPreallocBytes 是按 ContentLength 预分配的上限，超过就交给缓冲自己扩容。
const kongZstdMaxPreallocBytes = 64 << 20
