...
func TestKongTriggerVerifyKeepsTicketOnIncompleteEvidence(t *testing.T) {
	...
	// 探测记录是离线重算的原始证据：每份挑战恰好一条，重复写入会让同一份回答在统计里算两次。
	repo.mu.Lock()
	parts := map[int]int{}
	...
	n := len(repo.probes)
	repo.mu.Unlock()
	if n != len(KongFingerprintChallenges()) {
		t.Errorf("探测记录 %d 条，应与挑战数 %d 相等", n, len(KongFingerprintChallenges()))
	}
	for part, c := range parts {
		if c != 1 {
			t.Errorf("第 %d 份挑战记了 %d 条", part, c)
		}
	}
}
...
func TestKongTriggerVerifyRevokesWhenUpstreamReissues(t *testing.T) {
	...
	up := &kongStubUpstream{
		...
		answers: []*KongUpstreamAnswer{
			{Text: "1,2,3", EchoedState: strings.Repeat("z", 292), StatusCode: 200},
		},
	}
	svc := kongTestService(t, repo, up, accounts)
	const model = "gpt-5.6-sol"
...
func TestKongTriggerVerifyStopsWhenUpstreamReissued(t *testing.T) {
	...
	up := &kongStubUpstream{
		proxyState: KongTicketProxyState{Exists: true},
		answers:    []*KongUpstreamAnswer{{Text: "1,2,3", EchoedState: strings.Repeat("z", 292), StatusCode: 200}},
	}
	svc := kongTestService(t, repo, up, accounts)
	const model = "gpt-5.6-sol"
	svc.accept = KongTicketAccept{model: []string{model}}
...
// 回票是响应头里的证据，读流之前就到了：2xx 之后断流不影响"注入没被接受"这个结论。
// 非 2xx 的错误响应里带票不算——它说明不了注入有没有被接受。
func TestKongVerifyEchoedStateWinsOverReadError(t *testing.T) {
	const model = "gpt-5.6-sol"
	for _, tc := range []struct {
		name        string
		status      int
		...
		{"2xx 回票后断流", 200, true, 1},
		{"5xx 响应里带票", 502, false, 3},
	} {
		t.Run(tc.name, func(t *testing.T) {
			repo := newKongStubRepo()
			accounts := &kongStubAccounts{accounts: map[int64]*Account{}}
			...
			up := &kongStubUpstream{
				proxyState: KongTicketProxyState{Exists: true},
				answers:    kongVerifyAnswers(),
				echoState:  strings.Repeat("e", 780),
				echoErr:    io.ErrUnexpectedEOF,
				echoStatus: tc.status,
			}
			svc := kongTestService(t, repo, up, accounts)
			svc.accept = KongTicketAccept{model: []string{model}}
			current := kongSeedCurrentTicket(repo, model, time.Now())
			...
			if got := out.Steps[0].Revoked; got != tc.wantRevoked {
				t.Errorf("撤销 = %v，应为 %v（%+v）", got, tc.wantRevoked, out.Steps[0])
			}
			up.mu.Lock()
			calls := up.challengeCall
			up.mu.Unlock()
			if calls != tc.wantCalls {
				t.Errorf("挑战发了 %d 份，应为 %d", calls, tc.wantCalls)
			}
			wantStatus := KongTicketStatusVerified
			if tc.wantRevoked {
				wantStatus = KongTicketStatusRejected
			}
			if st, _ := repo.TicketStatus(context.Background(), current.ID); st != wantStatus {
				t.Errorf("票状态 = %s，应为 %s", st, wantStatus)
			}
		})
	}
}

// 当前票进入刷新窗口、缓存里有够格的候选时：业务照常拿到当前票，候选在后台真的被验掉，且不去取新票。
// 账号不可调度时后台不发挑战。
func TestKongRefreshWindowVerifiesCandidateInBackground(t *testing.T) {
	const model = "gpt-5.6-sol"
	for _, tc := range []struct {
		name       string
		ready      bool
		...
		{"账号可调度", true, KongTicketStatusVerified},
		{"账号不可调度", false, KongTicketStatusUnverified},
	} {
		t.Run(tc.name, func(t *testing.T) {
			repo := newKongStubRepo()
			accounts := &kongStubAccounts{accounts: map[int64]*Account{}}
			account := kongVerifyTestAccount()
			account.Schedulable = tc.ready
			accounts.set(account)
			up := &kongStubUpstream{proxyState: KongTicketProxyState{Exists: true}, answers: kongVerifyAnswers()}
			svc := kongTestService(t, repo, up, accounts)
			svc.accept = KongTicketAccept{model: []string{"gpt-6-astra", "gpt-5.6-sol"}}

			current := kongSeedCurrentTicket(repo, model, time.Now())
			soon := time.Now().Add(2 * time.Minute)
			current.ExpiresAt = soon
			repo.tickets[current.ID].ExpiresAt = soon
			candidate := kongSeedCandidate(repo, model, 21, time.Minute)

			grant, err := svc.EnsureTicket(context.Background(), 1, model)
			if err != nil || grant == nil || !grant.Allowed || grant.TicketID != current.ID {
				t.Fatalf("刷新窗口内业务应当照常拿到当前票：grant=%+v err=%v", grant, err)
			}

			deadline := time.Now().Add(5 * time.Second)
			for {
				repo.mu.Lock()
				status := repo.tickets[candidate.ID].Status
				repo.mu.Unlock()
				if status == tc.wantStatus && (tc.wantStatus != KongTicketStatusUnverified || time.Now().After(deadline.Add(-4*time.Second))) {
					break
				}
				if time.Now().After(deadline) {
					t.Fatalf("候选状态 = %s，应为 %s", status, tc.wantStatus)
				}
				time.Sleep(10 * time.Millisecond)
			}
			up.mu.Lock()
			calls, fetches := up.challengeCall, up.fetchCalls
			up.mu.Unlock()
			if tc.ready && calls == 0 {
				t.Error("后台应当真的发了挑战")
			}
			if !tc.ready && calls != 0 {
				t.Errorf("账号不可调度时不该发挑战，实发 %d 份", calls)
			}
			// 不可调度时连验证流程都不该进：刷新窗口内每个请求都会起一次后台任务，进了验证再被前提
			// 复核挡下，每次都会留一条验证事件。
			if n := kongCountEvents(repo, KongEventVerify); !tc.ready && n != 0 {
				t.Errorf("账号不可调度时不该写验证事件，实写 %d 条", n)
			}
			if fetches != 0 {
				t.Errorf("手上有候选时不该去取新票，实取 %d 次", fetches)
			}
		})
	}
}
