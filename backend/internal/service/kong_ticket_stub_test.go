...
// VerifiedTicketsElsewhere 与生产同条件：别的账号、同模型、verified、未过期、未 skip，**不按账号
// 截断**，且带回归因分布（调用方要按当前白名单重判）。按过期时刻降序，与生产的 ORDER BY 一致。
...
// 资格的条件（含 `skip_until_new = FALSE`）必然更新 0 行。桩若不照做，"非目标结论被自身的批量 skip
// 作废"这类回归会在测试全绿的情况下溜过去。
