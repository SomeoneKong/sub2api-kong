...
func (s *KongTicketService) ensureTicketOpts(ctx context.Context, accountID int64, model string, allowHandoff bool) (*KongTicketGrant, error) {
	return s.ensureTicket(ctx, accountID, model, false, allowHandoff)
}

// ensureTicket 是 EnsureTicket 的实现。
//
// manual 为真时跳过静默与冷却（见 KongScheduleInput.IgnoreWindow），只由人工触发置真。
func (s *KongTicketService) ensureTicket(ctx context.Context, accountID int64, model string, manual, allowHandoff bool) (*KongTicketGrant, error) {
	...
	case KongActionInjectAndPrefetch:
		if manual {
			...
			return s.runTaskAndWait(ctx, account, cfg, model, decision.Action, manual)
		}
	...
	case KongActionInjectAndVerifyCandidate:
		if manual {
			...
			return s.runTaskAndWait(ctx, account, cfg, model, KongActionVerifyCandidate, manual)
		}
...
func (s *KongTicketService) startTask(ctx context.Context, task *kongTicketTask, account *Account, model, action string, manual bool) {
	detached, cancel := context.WithTimeout(context.WithoutCancel(ctx), kongTaskBudget)
	go func() {
		defer cancel()
		defer s.releaseTask(account.ID, task)
		id, revived, err := s.runTask(detached, account, model, action, task.ticketEgress, manual)
...
func (s *KongTicketService) runTask(ctx context.Context, account *Account, model, action, claimedEgress string, manual bool) (ticketID int64, revivedID int64, err error) {
	...
	if action == kongActionRevive {
		return s.reviveAndVerify(ctx, fresh, freshCfg, model, taskStartedAt)
	}
	...
	if needsTicketEgress {
		if current := KongEgressKey(freshCfg.Egress, freshCfg.ProxyID); current != claimedEgress {
			return 0, 0, fmt.Errorf("票据出口已从 %s 改为 %s，本任务持有的是旧锁", claimedEgress, current)
		}
	}

	switch decision.Action {
	case KongActionVerifyCandidate, KongActionInjectAndVerifyCandidate:
		// 刷新窗口里的候选验证（当前票照常服务、后台把候选验掉）重算后得到的是 InjectAndVerifyCandidate，
		// 与无票时同走这一条。那个决策不看账号可调度——当前票还能用，不必拒服——而验证会主动发挑战，
		// 所以在这里补判。
		if decision.Action == KongActionInjectAndVerifyCandidate && !in.AccountReady {
			return 0, 0, fmt.Errorf("账号当前不可调度，暂不验证候选")
		}
	...
	case KongActionInject:
		...
		if in.CurrentTicketID != 0 {
			return in.CurrentTicketID, 0, nil
		}
		return 0, 0, fmt.Errorf("重查后无可用票")
	default:
		// 前提已不成立（静默不够、冷却中、出口失效、账号不可调度）。什么都不做是正确的：
		// 这一次拒服，下一次请求重新判定。
		return 0, 0, fmt.Errorf("重查后不再满足取票条件: %s", decision.DenyReason)
	}
}

// reviveAndVerify 复位一张未过期的已拒票，并以它为目标重验。只由人工刷新的 kongActionRevive 任务调用。
//
// **复位必须在持有槽位之后做**，不能放在触发入口：入口那时还没拿到槽位，若有同模型任务在跑，它收尾
// 时的批量跳过会按自己的边界把刚复位的票标成 skip——那张票此后既选不中、也不再是 rejected，连下一次
// 人工触发都救不回来。
//
// 只在重读之后仍是 full 且账号可调度时复位：认领与重读之间模式可能已被切走，那时复位并重验一张已拒票
// 既违背退出保护的意图，也白烧挑战额度；不可调度时复位出来的候选没人验，只是把一张已拒票悄悄变回候选。
//
// 复位成功就**直接验证那一张**，不泛选候选：泛选可能挑到更早的另一张票，而调用方要据 revivedID 报
// "正在复用该票重验"；它也绕开 observed 的最小年龄限制——那条限制压的是被动票的**自动**验证频率，对
// 人工指名的重验没有意义。没有可复位的票时什么都不做，由调用方接着走正常决策。
func (s *KongTicketService) reviveAndVerify(ctx context.Context, fresh *Account, cfg KongTicketConfig, model string, taskStartedAt time.Time) (ticketID int64, revivedID int64, err error) {
	if cfg.Mode != KongTicketModeFull || !fresh.IsSchedulable() {
		return 0, 0, nil
	}
	revived, err := s.repo.ReviveRejectedCandidate(ctx, fresh.ID, model)
	if err != nil {
		// 复位失败不该让人工刷新失败：调用方照常按正常决策走（多半是去取新票）。
		s.logEvent(ctx, &KongTicketEvent{
	...
	ticketID, err = s.verifyTicket(ctx, fresh, cfg, model, revived.ID, revived.State, revived.Source,
		revived.ExpiresAt, kongCaptureOf(revived), taskStartedAt, false, nil)
	return ticketID, revivedID, err
}
...
func (s *KongTicketService) startPrefetch(ctx context.Context, account *Account, cfg KongTicketConfig, model string) {
	...
	s.startTask(ctx, task, account, model, KongActionInjectAndPrefetch, false)
}
...
func (s *KongTicketService) startVerifyCandidate(ctx context.Context, account *Account, model string) {
	...
	s.startTask(ctx, task, account, model, KongActionVerifyCandidate, false)
}
...
func (s *KongTicketService) prepareOrHandOff(ctx context.Context, account *Account, cfg KongTicketConfig, model, action string) (*KongTicketGrant, error) {
	if !s.handoffWorthIt(ctx, account.ID, model) {
		return s.runTaskAndWait(ctx, account, cfg, model, action, false)
	}
	...
	switch outcome {
	case kongClaimFresh:
		s.startTask(ctx, task, account, model, action, false)
...
func (s *KongTicketService) runTaskAndWait(ctx context.Context, account *Account, cfg KongTicketConfig, model string, action string, manual bool) (*KongTicketGrant, error) {
	// 候选验证与已拒票重验走**流量出口**，不占票据出口的静默，所以不该抢那个槽位——否则两个都配
	// `none` 的账号会争用同一个 "none" 锁，一个在验证、另一个被拒为 egress_busy。
	ticketEgress := KongEgressKey(cfg.Egress, cfg.ProxyID)
	if action == KongActionVerifyCandidate || action == kongActionRevive {
		ticketEgress = kongVerifyOnlySlot(account.ID)
	}
	task, outcome := s.claimTask(account.ID, model, ticketEgress)
	switch outcome {
	...
	s.startTask(ctx, task, account, model, action, manual)
	return s.awaitTask(ctx, task, account.ID, model)
}
...
// kongActionRevive 是人工刷新专用的任务动作：持槽复位一张已拒票并重验它（见 reviveAndVerify）。
// 它不是调度决策的产物，只由 TriggerRefresh 直接起，与候选验证一样占验证专用槽位。
const kongActionRevive = "revive"
...
func (s *KongTicketService) verifyTicket(ctx context.Context, account *Account, cfg KongTicketConfig, model string, ticketID int64, state, source string, expiresAt time.Time, capture *kongCaptureFacts, taskStartedAt time.Time, reverify bool, fused *kongFusedSample) (int64, error) {
	...
	for i, challenge := range KongFingerprintChallenges() {
		...
		// 回票的处置排在 runErr **之前**：响应头在读流之前就到了，2xx 之后读流出错不影响"上游又下发了票"
		// 这条证据。非 2xx 的响应不算——它说明不了注入有没有被接受（口径见 kongIsSuccessStatus）。
		if answer != nil && answer.EchoedState != "" && kongIsSuccessStatus(answer.StatusCode) {
...
//
// 另有一个前置动作：先复位并重验一张未过期的已拒票（见 reviveAndVerify）。它在改过白名单或阈值
// 之后最有价值——同一张票的证据在新判据下可能就合格了，而重验走流量出口、不消耗票据出口的静默，
// 比取一张新票便宜得多。也因为只走流量出口，票据出口那几条结构性约束管不到它，账号可调度即可。
func (s *KongTicketService) TriggerRefresh(ctx context.Context, accountID int64, model string) (*KongManualRefresh, error) {
	...
	fill := func(grant *KongTicketGrant) *KongManualRefresh {
	...
	// 先占验证专用槽位复位并重验一张已拒票。重验只走流量出口，**不能先按取票条件决策**：票据出口为
	// none、票据代理失效或出口被别的账号占着，约束的都只是取新票，挡在这里会让本该零取票完成的重验
	// 直接拒服。没有可复位的票时这个任务什么都不做，接着按人工口径走正常决策（跳过静默与冷却）。
	cfg, _ := ParseKongTicketConfig(account.Extra)
	if cfg.Mode == KongTicketModeFull && account.IsSchedulable() {
		grant, err := s.runTaskAndWait(ctx, account, cfg, model, kongActionRevive, true)
		if err != nil {
			return nil, err
		}
		if grant.RevivedTicketID != 0 {
			return fill(grant), nil
		}
	}
	grant, err := s.ensureTicket(ctx, accountID, model, true, false)
	if err != nil {
		return nil, err
	}
	return fill(grant), nil
