...
type KongUpstreamAttempt struct {
	...
	// 之后只读。
	ResponseID string
	// RetiredResponseIDs 是被本轮覆盖掉的、尚未结束的前几轮的响应 id（见 kongWSInheritRetired）。
	// 它们迟到的终端事件不得释放本轮。与 ResponseID 一样在发布前写好、之后只读。
...
func (g *KongTicketGateway) PrepareUpstream(ctx context.Context, req *http.Request, account *Account) (*KongUpstreamAttempt, error) {
	...
	// **不产出模型内容的端点不要票**（计 token、独立搜索，清单与理由见 kongNonGeneratingPathSuffixes）：
	// 没有可被降智的东西，门控它只会在无票时白拒一次请求（无谓拒服）。这条豁免要先于模型判定：判不出
	// 模型的拒服同样是在保护一份并不存在的输出，而计数通路没有 failover 与拒服呈现，拒在这里只会落成
	// 一个空响应。
	nonGenerating := kongIsNonGeneratingUpstreamPath(req.URL)
	result, err := g.gatedModelOf(req)
	if err != nil {
		if !g.ProtectsAccount(account) || nonGenerating {
			// 这个账号不在保护范围内（或这次发送不产出内容），读不出模型也无所谓——本来就不注入、不判定。
			return nil, nil
		}
	...
	// 不产出内容的端点（豁免理由见开头）仍然返回一个 attempt（Grant 为 nil）而不是 nil：那样 AfterUpstream
	// 照旧收响应头里的票（observed 票是免费的，不消耗任何出口静默），只是不做"上游是否接受注入"的判定
	// ——本来也没注入。
	//
	// embeddings 同样不产出模型内容，但它有自己的模型白名单，受门控的 codex 文本模型到不了那里。
	if nonGenerating {
		return attempt(nil), nil
	}
...
// kongIsSuccessStatus 报告一个上游响应是否成功（2xx）。"上游回发 state = 注入未被接受"这条证据只在成功
// 响应上成立，交付判定与验证挑战共用这一个口径。
func kongIsSuccessStatus(code int) bool {
	return code >= 200 && code < 300
}

// kongNonGeneratingPathSuffixes 是不产出模型内容的端点。
//
// 按后缀匹配而不是全等：API Key 账号可以配自定义 base URL，端点会长成 `/api/v1/responses/input_tokens`
// 这样。
//   - `/responses/input_tokens`：Anthropic 侧的 count_tokens 与 Responses 侧的 input_tokens 共用它。
//   - `/alpha/search`：codex 的独立搜索（web.run）。返回的是检索结果与网页的渲染文本；请求不带
//     x-codex-turn-state，上游也不回发，票在这条路上注入不进去、也拿不到交付证据，门控它只会让受保护
//     账号在无票时拒掉搜索。PAT 账号的搜索被改写成 /responses + hosted web_search，那是真正的生成，
//     上游路径不同，照常门控。
var kongNonGeneratingPathSuffixes = []string{"/responses/input_tokens", "/alpha/search"}
...
func kongIsNonGeneratingUpstreamPath(u *url.URL) bool {
	...
	return kongIsNonGeneratingPath(u.Path)
}

// kongIsNonGeneratingPath 按路径判定。上游 URL 与入站端点共用这一份清单。
func kongIsNonGeneratingPath(path string) bool {
	path = strings.TrimSuffix(path, "/")
	for _, suffix := range kongNonGeneratingPathSuffixes {
...
//
// **只有 2xx 响应里的 state 算证据。** 错误响应（401 / 429 / 5xx）里带 state 说明不了注入有没有被接受；
// 据它撤票会作废一张好票，还会把真实的上游错误换成本地交付拦截——调用方拿不到原响应，换号、限流与
// 凭据故障的处置全被跳过，调度上报又会把它当成本地拦截豁免掉。收票与记特征不受此限。
func (g *KongTicketGateway) AfterUpstream(ctx context.Context, account *Account, attempt *KongUpstreamAttempt, resp *http.Response) error {
	...
	if attempt.Grant == nil || state == "" || !kongIsSuccessStatus(resp.StatusCode) {
		return nil
	}
...
//
// 上游除了在 `response.metadata` 事件里给 state，也会在与它那条连接的**握手响应头**里给一个（三条
// WS 通路都会把它采纳成连接级的 turnState）。只收事件那一路等于白扔一批免费样本——observe 模式靠
// 样本判档，full 模式靠它在缓存里备着票。
//
...
//
// ⚠️ 不能只放行 `preparing`、以"其余换号只是把同一个结论在每个账号上重演"为由直接拒服：那把账号级
// 条件当成了全局条件。一个账号因 window_closed 拒服时，同分组另一个账号完全可能已经有合格票，不换号
// 就是无谓拒服，与放行降智同级。真正全局的情况（所有账号都没票）由 failover 框架自然耗尽，不需要在
// 票据层提前判定。
//
...
//     那次换号。
//
// ⚠️ `egress_busy` 不算"值得等"，尽管看上去"别的任务占着，马上释放"：占着
...
// `*KongErrTicketDenied` 去豁免健康度。转换时丢掉原类型的话，豁免只能靠 reason 字符串反查，带自由
// 文本的原因一律漏掉。
