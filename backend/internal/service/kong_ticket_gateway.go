...
func (g *KongTicketGateway) PrepareUpstream(ctx context.Context, req *http.Request, account *Account) (*KongUpstreamAttempt, error) {
	...
	// 计数请求（无谓拒服）。这条豁免要先于模型判定：判不出模型的拒服同样是在保护一份并不存在的输出，
	// 而计数通路没有 failover 与拒服呈现，拒在这里只会落成一个空响应。
	...
	// 计 token 的端点（豁免理由见开头）仍然返回一个 attempt（Grant 为 nil）而不是 nil：那样 AfterUpstream
