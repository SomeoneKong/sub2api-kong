...
func TestKongPaceConfig_SingleDocumentOnly(t *testing.T) {
	if _, err := ParseKongPaceConfig([]byte(kongPaceTestYAML + "# trailing comment\n")); err != nil {
		t.Fatalf("trailing comment must be accepted: %v", err)
	}
	for name, tail := range map[string]string{
		"second document": "---\nenabled: false\n",
		"broken tail":     "---\nenabled: [\n",
	} {
		if _, err := ParseKongPaceConfig([]byte(kongPaceTestYAML + tail)); err == nil {
			t.Errorf("%s must be rejected", name)
		}
	}
}

func TestKongPaceLinePenalty_HugeMaxStaysFinite(t *testing.T) {
	q, at := kongPaceLinePenalty(40, KongPaceSoftLine{Start: 25, Full: 50}, 1e308)
	if math.IsInf(q, 0) || math.IsNaN(q) || q > 1e308 || at || math.Abs(q-6e307) > 1e293 {
		t.Fatalf("penalty must stay finite and below qmax: q=%v at=%v", q, at)
	}
}
...
func TestKongPaceFactsTick_RestoreFailureRetriesWithoutOverwriting(t *testing.T) {
	reset := kongPaceT0.Add(72 * time.Hour)
	rows := []KongPaceAccountRow{kongPaceRow(1, "pro", 60, reset, kongPaceT0.Add(-time.Minute))}
	p, _, store := kongPaceTestComponent(t, rows, kongPaceTestYAML)
	history, err := json.Marshal(kongPaceAccountState{Plan: "pro", HasWindow: true, WindowMinutes: 10080, ResetAt: reset,
		MaxUsed: 60, ReadingAt: kongPaceT0.Add(-time.Minute), SeenReadingAt: kongPaceT0.Add(-time.Minute),
		ProcessedAt: kongPaceT0.Add(-time.Minute), TrackedSince: kongPaceT0.Add(-48 * time.Hour),
		Rises: []kongPaceRise{{At: kongPaceT0.Add(-time.Hour), Points: 50, ReadingAt: kongPaceT0.Add(-time.Hour)}}})
	if err != nil {
		t.Fatal(err)
	}
	store.saved = map[int64][]byte{1: history}
	store.loadErr = errors.New("redis timeout")

	p.factsTick()
	if p.snapshot.Load() != nil || string(store.saved[1]) != string(history) {
		t.Fatal("a failed restore must not build fresh state over the stored history")
	}
	store.loadErr = nil
	p.factsTick()
	f := p.snapshot.Load().accounts[1]
	if got := kongPaceIncrease(&f.State, kongPaceT0, 6*time.Hour); got != 50 {
		t.Fatalf("restored history must be kept, inc6=%v", got)
	}
}

func TestKongPaceFactsTick_RetriesFailedDeletes(t *testing.T) {
	reset := kongPaceT0.Add(72 * time.Hour)
	...
	store.saveErr = nil
	p.factsTick()
	if _, ok := store.saved[2]; ok {
		t.Fatal("a delete that failed once must be retried")
	}

	// 待删的账号又出现了：不能在同一批里先写后删。
	repo.rows = rows
	p.factsTick()
	repo.rows = rows[:1]
	store.saveErr = errors.New("redis down")
	...
	p.factsTick()
	if _, ok := store.saved[2]; !ok {
		t.Fatal("an account that came back must keep its state")
	}
}
...
func TestKongPaceReorder_TiersSegmentsAndProbability(t *testing.T) {
	...
	out := d.reorder(in, openAILegacyUpstreamRateOrder{})
...
func TestKongPaceDecision_AppliedOnlyWhenASegmentIsPaced(t *testing.T) {
	...
	if out := d.reorder(mixed, openAILegacyUpstreamRateOrder{}); out[0].account.ID != 1 || out[1].account.ID != 9 {
	...
	d = svc.kongPaceBegin(context.Background(), nil, "h", "gpt", false)
	d.reorder(mixed, openAILegacyUpstreamRateOrder{})
	single := kongPaceAvailable(kongPaceOAuth(2, 10))
	d.reorder(single, openAILegacyUpstreamRateOrder{})
...
func TestKongPaceDecision_AtLineCountUsesSchedulingRate(t *testing.T) {
	reset := kongPaceT0.Add(72 * time.Hour)
	p, _, _ := kongPaceTestComponent(t, []KongPaceAccountRow{kongPaceRow(1, "pro", 40, reset, kongPaceT0), kongPaceRow(2, "pro", 40, reset, kongPaceT0)}, kongPaceTestYAML)
	p.factsTick()
	// 账号 2 近 6h 已用 50%，到线。
	...
	choose := func(rates openAILegacyUpstreamRateOrder) int64 {
		before := p.counters.atLineChosen.Load()
		d := svc.kongPaceBegin(context.Background(), nil, "h", "gpt", false)
		in := kongPaceAvailable(kongPaceOAuth(1, 10), kongPaceOAuth(2, 10))
		d.finalOrder(d.reorder(in, rates))
		d.acquired(2)
		d.finish()
		<-p.decisions
		return p.counters.atLineChosen.Load() - before
	}
	if got := choose(openAILegacyUpstreamRateOrder{}); got != 1 {
		t.Fatalf("without rate ordering the open account was an alternative, count=%d", got)
	}
	if got := choose(openAILegacyUpstreamRateOrder{enabled: true, rates: map[int64]float64{1: 1, 2: 2}}); got != 0 {
		t.Fatalf("a different scheduling rate is not an alternative, count=%d", got)
	}
	if got := choose(openAILegacyUpstreamRateOrder{enabled: true, rates: map[int64]float64{1: 0.5, 2: 0.5}}); got != 1 {
		t.Fatalf("the same scheduling rate is an alternative, count=%d", got)
	}
}
...
func TestKongPaceNilSafe(t *testing.T) {
	...
	if out := d.reorder(in, openAILegacyUpstreamRateOrder{}); len(out) != 1 {
