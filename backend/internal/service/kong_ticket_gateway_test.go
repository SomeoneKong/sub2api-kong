...
func TestKongGatewayAfterUpstreamDecision(t *testing.T) {
	cases := []struct {
		...
		// 错误响应里的 state 说明不了注入有没有被接受：原响应要交回调用方的错误处理。
		{name: "注入过 + 401 带 state = 不拦", attempt: &KongUpstreamAttempt{Model: "gpt-6-astra", Grant: &KongTicketGrant{Allowed: true, TicketID: 7}}, respState: "aaaaaaaa", status: 401},
		{name: "注入过 + 429 带 state = 不拦", attempt: &KongUpstreamAttempt{Model: "gpt-6-astra", Grant: &KongTicketGrant{Allowed: true, TicketID: 7}}, respState: "aaaaaaaa", status: 429},
		{name: "注入过 + 502 带 state = 不拦", attempt: &KongUpstreamAttempt{Model: "gpt-6-astra", Grant: &KongTicketGrant{Allowed: true, TicketID: 7}}, respState: "aaaaaaaa", status: 502},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
...
// 计 token 与独立搜索的端点不参与门控：它们不产出模型内容，门控只会在无票时白拒一次请求。
//
...
func TestKongNonGeneratingUpstreamPath(t *testing.T) {
	cases := map[string]bool{
		...
		// 独立搜索：OAuth 账号发往 ChatGPT Codex 后端，API Key 账号发往平台或自定义 base URL。
		"https://chatgpt.com/backend-api/codex/alpha/search": true,
		"https://api.openai.com/v1/alpha/search":             true,
		"https://relay.example.com/api/v1/alpha/search/":     true,
		"https://api.openai.com/v1/alpha/searches":           false,
		"https://api.openai.com/v1/alpha/search/stream":      false,
	}
...
// 受门控模型发往不产出内容的端点时：不要票、不注入，但仍返回 attempt，好让 AfterUpstream 收响应头里的
// 免费票（observed 票不消耗任何出口静默）。
func TestKongPrepareUpstreamSkipsNonGeneratingEndpoints(t *testing.T) {
	account := &Account{ID: 1, Extra: map[string]any{KongTicketModeKey: string(KongTicketModeFull)}}
	g := NewKongTicketGateway(&KongTicketService{repo: newKongStubRepo()}, []string{"gpt-6-astra", "gpt-6-sol"})
	for _, tc := range []struct{ url, body string }{
		{"https://api.openai.com/v1/responses/input_tokens", `{"model":"gpt-6-astra","input":"hi"}`},
		{"https://chatgpt.com/backend-api/codex/alpha/search", `{"id":"s","model":"gpt-6-sol","commands":{"search_query":[{"q":"x"}]}}`},
	} {
		...
		if err != nil {
			t.Fatalf("%s 不该被拒服，实得 %v", tc.url, err)
		}
		if attempt == nil {
			t.Fatalf("%s 仍要返回 attempt，否则 AfterUpstream 不会收响应头里的免费票", tc.url)
		}
		if attempt.Grant != nil {
			t.Errorf("%s 不该注入票", tc.url)
		}
		if req.Header.Get(openAICodexTurnStateHeader) != "" {
			t.Errorf("%s 不该写票据请求头", tc.url)
		}
	}
}
...
// 不产出内容的端点判不出模型时也不拒服。
//
// 计数通路没有 failover 与拒服呈现：拒在这里，handler 只记一条日志，客户端拿到的是一个空的 200。
// 而这次发送不产出任何模型内容，拒服保护的是一份并不存在的输出。
func TestKongPrepareUpstreamNonGeneratingNeverDenies(t *testing.T) {
	account := &Account{ID: 1, Extra: map[string]any{KongTicketModeKey: string(KongTicketModeFull)}}
	g := NewKongTicketGateway(&KongTicketService{repo: newKongStubRepo()}, []string{"gpt-6-astra"})
	for name, body := range map[string]string{
		"重复 model 键": `{"model":"gpt-6-astra","model":"gpt-6-astra","input":"hi"}`,
		"model 非字符串": `{"model":7,"input":"hi"}`,
	} {
		for _, endpoint := range []string{
			...
			t.Run(name+" "+endpoint, func(t *testing.T) {
				...
				if _, err := g.PrepareUpstream(context.Background(), req, account); err != nil {
					t.Fatalf("不产出内容的端点不该被拒服，实得 %v", err)
				}
				if req.Header.Get(openAICodexTurnStateHeader) != "" {
					t.Error("不产出内容的端点不该写票据请求头")
				}
			})
		}
	}
	// 同样的载荷发往生成端点仍按原规则拒服。
	req, err := http.NewRequest(http.MethodPost, "https://api.openai.com/v1/responses",
		strings.NewReader(`{"model":"gpt-6-astra","model":"gpt-6-astra","input":"hi"}`))
	if err != nil {
		t.Fatalf("构造请求: %v", err)
	}
	if _, err := g.PrepareUpstream(context.Background(), req, account); !KongIsTicketDenied(err) {
		t.Fatalf("生成端点判不出模型必须拒服，实得 %v", err)
	}
}
...
// 缺特征守卫对不产出内容的端点闭嘴：独立搜索的请求不带 state、上游也不回发，"门控请求必有 state"
// 的前提在那里不成立，照喊会把日志淹掉，真正漏接的通路反而看不见。同样的空特征落在生成端点上仍要喊。
func TestKongWarnMissingRequestFeaturesSkipsNonGeneratingEndpoints(t *testing.T) {
	var buf bytes.Buffer
	previous := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&buf, nil)))
	t.Cleanup(func() { slog.SetDefault(previous) })

	g := NewKongTicketGateway(&KongTicketService{repo: newKongStubRepo()}, []string{"gpt-6-sol"})
	g.WarnMissingRequestFeatures("gpt-6-sol", "/v1/alpha/search", false, nil)
	g.WarnMissingRequestFeatures("gpt-6-sol", "/backend-api/codex/alpha/search", false, nil)
	if strings.Contains(buf.String(), "门控请求没有采到任何特征") {
		t.Fatalf("独立搜索不该触发缺特征告警：%s", buf.String())
	}
	g.WarnMissingRequestFeatures("gpt-6-sol", "/v1/responses", false, nil)
	if !strings.Contains(buf.String(), "门控请求没有采到任何特征") {
		t.Fatal("生成端点的门控请求没有特征时仍要告警")
	}
}
