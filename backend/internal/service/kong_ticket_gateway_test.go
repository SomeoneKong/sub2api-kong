...
func TestKongPrepareUpstreamTokenCountingNeverDenies(t *testing.T) {
	...
	for name, body := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			req, err := http.NewRequest(http.MethodPost,
				"https://api.openai.com/v1/responses/input_tokens", strings.NewReader(body))
			if err != nil {
				t.Fatalf("构造请求: %v", err)
			}
