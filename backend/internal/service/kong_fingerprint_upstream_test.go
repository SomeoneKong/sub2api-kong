...
func TestKongReadCodexSSEReportedModelPrecedence(t *testing.T) {
	...
	cases := []struct {
		...
		// 反方向：早期不一致、最终一致时也要取最终的，否则会把一致误判成对不上。
...
// 上游回报的模型名带快照后缀时算同一个模型；短基名不得吞掉以它为前缀的另一个模型。
func TestKongModelSnapshotMatch(t *testing.T) {
	cases := []struct {
		base, reported string
		want           bool
	}{
		{"gpt-6-sol", "gpt-6-sol", true},
		{"gpt-6-sol", "GPT-6-SOL", true},
		{"gpt-5.4-mini", "gpt-5.4-mini-2026-03-17", true},
		{"gpt-6", "gpt-6-2026-03-17", true},
		{"gpt-6", "gpt-6-astra", false},
		{"gpt-6", "gpt-6-luna", false},
		{"gpt-5.6-sol", "gpt-5.6-sol-mini", false},
		{"gpt-6-sol", "gpt-6-luna", false},
		{"gpt-6-sol", "gpt-6-sol-", false},
	}
	for _, tc := range cases {
		if got := kongModelSnapshotMatch(tc.base, tc.reported); got != tc.want {
			t.Errorf("kongModelSnapshotMatch(%q, %q) = %v, want %v", tc.base, tc.reported, got, tc.want)
		}
	}
}
