...
//
// **出口隔离只对 full 校验。** off / observe 不主动取票，出口是否合并对它们没有影响；而流量代理可以在
// 不经票据配置的情况下被改成与票据代理相同，那时若照样校验，账号就无法从 full 退回 off / observe——
// 退出保护这个动作被一份已经不起作用的出口配置挡住。结构性错误（未知模式、未知出口、proxy 缺 id）
// 仍对所有模式校验。
