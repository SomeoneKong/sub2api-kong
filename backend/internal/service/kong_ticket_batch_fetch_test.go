...
// kongRealSolAnswerText 取 golden 里单份概率最高的一条真实 5.6-sol 回答。
//
// "一份就够判定"的用例要用真实回答：合成数字序列不是任何模型的输出，它的归因取决于库里恰好有
// 哪些候选，换一版校准资料就可能掉到门槛以下，用例测的就不再是融合逻辑本身。
func kongRealSolAnswerText(t *testing.T) string {
	t.Helper()
	var best kongFPGoldenRow
	for _, row := range kongFPLoadGolden(t) {
		if row.Best == kongBatchSol && row.BestP > best.BestP {
			best = row
		}
	}
	if best.BestP < 0.99 {
		t.Fatalf("golden 里没有单份就足够确定的 5.6-sol 回答（最高 %.4f）", best.BestP)
	}
	return best.Text
}
...
func TestKongFusedFetchNeedsNoChallenge(t *testing.T) {
	up := &kongStubUpstream{
		...
		fetchState: strings.Repeat("a", 292),
		answers:    kongVerifyAnswers(),
		// 取票请求要回的正文用一份真实的 sol 回答：单份就归因到 sol，而 sol 的白名单含 sol。
		fusedText: kongRealSolAnswerText(t),
	}
	svc, repo := kongFusedSetup(t, up, false)
	// 白名单按生产默认给 sol 配上 astra 与 5.5；这条用例要测的是"够用时不再发挑战"，不是门槛本身。
...
func TestKongFusedSettlesNonLeaderTicket(t *testing.T) {
	...
	// 同上：一份真实 sol 回答就过门槛；astra 这里也放宽到接受 sol，让两个模型都靠融合判定。
