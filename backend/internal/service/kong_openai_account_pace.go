...
func (p *KongOpenAIAccountPace) factsTick() {
	...
	p.persist(ctx, deleted)
	p.publish(ctx, loaded, snap, now)
	p.maybeSweepDecisions(loaded, now)
	p.maybeLogSummary(loaded, snap, now)
}

// restoreStates 在第一拍从 Redis 载入各账号状态。读不到时从空开始：消耗记录只会少计。
func (p *KongOpenAIAccountPace) restoreStates(ctx context.Context) {
	p.statesReady = true
	raw, err := p.store.LoadAccountStates(ctx)
	if err != nil {
		slog.Warn("kong pace: 载入账号状态失败，从空开始", "error", err)
		return
	}
...
func (p *KongOpenAIAccountPace) persist(ctx context.Context, deleted []int64) {
