...
// 账号指纹测试的归因：ModelTrace 官方口径的 Go 移植。设计见 DESIGN-openai-fingerprint-test.md。
//
// 结论只看最像哪个模型、把握够不够，但算法必须照官方来、不自创变体——偏差不会报错，只会
// 表现成测不出结论或测错模型。移植正确性由 golden file 回归测试锁住（kong_fingerprint_test.go）。
//
...
//
// NaN 会让后面每个比较都为假——「概率 >= 置信度」恒不成立，表现成指纹测试永远不下结论，而没有任何
// 一步报错。所以要在产出结论之前把它挡住。
...
func KongFingerprintAttribute(answers []KongFingerprintAnswer, bank *KongFingerprintBank) (*KongFingerprintResult, error) {
	...
	// 任何比较都为假——「概率 >= 置信度」恒不成立，表现成永远不下结论且一处不报错。
