...
//
// ⚠️ "只有 preparing 可 failover，其余换号救不了"这个假设把账号级条件当成了全局条件：静默与冷却是
// 按**该账号的票据出口**累积的，出口配置也是账号自己的。同分组另一账号可能已持有合格票，不换号就是
// 无谓拒服，与放行降智同级。真正全局的情况（所有账号都没票）由 failover 框架自然耗尽，不在这一层判。
...
//
// 靠一张固定字符串清单反查的话，`PrepareUpstream` 产出的 `ensure_failed: <err>` /
// `model_undeterminable: <reason>` 会一律漏掉：调度豁免失效，本地拒服被计入账号错误率 EWMA，
// "票越缺、账号越被判坏"。所以身份认类型 + 受控前缀。
...
// 别家**最晚过期**的那张按当前白名单不合格、更早的一张仍合格时，照样交接。
//
// 接手判定与别家自己挑当前票必须是同一个结论：别家此刻正用那张旧票服务，这边却据最新那张判它
// 接不了手，请求会原地等一整轮取票与验证。
func TestKongHandsOffWhenOlderElsewhereTicketStillAccepted(t *testing.T) {
	const model = "gpt-6-astra"
	repo, _, svc := kongManualFixture(t, model)
	kongSeedElsewhereTicket(repo, 2, model, 95, map[string]float64{model: 0.97})
	kongSeedElsewhereTicket(repo, 2, model, 94, map[string]float64{"gpt-5.5": 0.99})
	later := time.Now().Add(50 * time.Minute)
	for _, tk := range repo.current[kongStubKey(2, model)] {
		if tk.ID == 94 {
			tk.ExpiresAt = later
		}
	}
	repo.tickets[94].ExpiresAt = later

	current, err := svc.currentTicket(context.Background(), 2, model)
	if err != nil || current == nil || current.ID != 95 {
		t.Fatalf("别家自己挑的应是那张仍合格的旧票：%+v, %v", current, err)
	}
	if !svc.handoffWorthIt(context.Background(), 1, model) {
		t.Fatal("别家有合格票，交接值得做")
	}
}
