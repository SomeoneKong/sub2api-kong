...
// reorder 在第 2 层每一轮 shuffleWithinSortGroups 之后调用：同一 accounts.priority 连续段内，
// 段里全是 OpenAI OAuth 账号时，未到线的在前、到线的在后，各自按权重不放回加权随机；混有其他
// 类型账号的段保持原顺序。未应用（关闭、快照缺失或过期）时照样算出节奏顺序记入决策，但返回原顺序。
func (d *kongPaceDecision) reorder(available []accountWithLoad) []accountWithLoad {
	...
	for i, item := range available {
		...
		views[i].Rate = acc.BillingRateMultiplier()
	}
...
func (d *kongPaceDecision) countAtLineChoice() {
	...
	for _, v := range views {
		if v.Paced && v.Priority == chosen.Priority && v.CompactTier == chosen.CompactTier && v.Rate == chosen.Rate && !v.AtLine {
