...
func TestKongRefreshTaskSkipsReviveAfterLeavingFull(t *testing.T) {
	...
	_, revivedID, _ := svc.runTask(context.Background(), claimed, model, kongActionRevive,
		kongVerifyOnlySlot(1), true)
...
// 复位重验只走流量出口，票据出口的状态不得挡住它。
//
// 票据出口为 none、票据代理失效、出口被别的账号占着，约束的都只是取新票。先按取票条件决策的话，
// 这三种情形会在复位之前就以 no_ticket_source / egress_unusable / egress_busy 拒服——本该零取票完成
// 的重验根本没机会开始。
func TestKongTriggerRefreshRevivesDespiteTicketEgress(t *testing.T) {
	const model = "gpt-5.6-sol"
	cases := []struct {
		name  string
		setup func(account *Account, up *kongStubUpstream, svc *KongTicketService) func()
	}{
		{"none", func(account *Account, _ *kongStubUpstream, _ *KongTicketService) func() {
			account.Extra[KongTicketEgressKey] = string(KongTicketEgressNone)
			return func() {}
		}},
		{"proxy_unusable", func(account *Account, up *kongStubUpstream, _ *KongTicketService) func() {
			account.Extra[KongTicketEgressKey] = string(KongTicketEgressProxy)
			account.Extra[KongTicketProxyIDKey] = int64(200)
			up.proxyState = KongTicketProxyState{Exists: false}
			return func() {}
		}},
		{"egress_busy", func(_ *Account, _ *kongStubUpstream, svc *KongTicketService) func() {
			task, claim := svc.claimTask(3, model, KongEgressKey(KongTicketEgressDirect, nil))
			if claim != kongClaimFresh {
				t.Fatal("构造出口占用失败")
			}
			return func() { svc.releaseTask(3, task) }
		}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if out.RevivedTicketID != 7 {
				t.Fatalf("应当复位并重验票 7，得到 %+v", out)
			}
			if up.fetchCalls != 0 {
				t.Errorf("复位重验不该取新票，实际取了 %d 次", up.fetchCalls)
			}
			if !out.Allowed || out.TicketID != 7 {
				t.Errorf("重验通过后应当授予该票，得到 %+v", out)
			}
		})
	}
}

// 没有可复位的票时，手工刷新照旧受取票的结构性约束：票据出口为 none 就拒服，不取票。
func TestKongTriggerRefreshWithoutRevivableKeepsFetchConstraints(t *testing.T) {
	repo := newKongStubRepo()
	accounts := &kongStubAccounts{accounts: map[int64]*Account{}}
	accounts.set(kongTestAccount(1, KongTicketModeFull, KongTicketEgressNone))
	...
	if out.RevivedTicketID != 0 || out.DenyReason != KongDenyNoTicketSource {
		t.Errorf("没有可复位的票时应以 no_ticket_source 结束，得到 %+v", out)
	}
	if up.fetchCalls != 0 {
		t.Errorf("票据出口为 none 时不该取票，实际取了 %d 次", up.fetchCalls)
	}
}
