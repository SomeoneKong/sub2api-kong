...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	if useHTTPBridge {
		...
		for turn := 1; ; turn++ {
			...
			// [kong] bridge 的出站请求头是连接级的 turn-state，帧内 client_metadata 才是客户端这一轮带来的
			// 那一份：客户端那一项以本轮帧的观测为准（见 MergeFrameClientState）。
			if result != nil {
				result.KongRequestFeatures = currentBridgePayload.kongFeatures.MergeFrameClientState(result.KongRequestFeatures)
			}
