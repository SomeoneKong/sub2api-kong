...
//
// **归因判据不在这里**——它在 service 侧的 KongTicketAccept 里，只有一份。采纳规则是「白名单内概率
// 求和」，SQL 与 Go 各写一遍就是两套规则，出入了也不会报错。
//
...
// VerifiedTicketsElsewhere 列别的账号在同一模型上此刻可用的票。
//
// 条件与 VerifiedTickets 一致（verified、未过期、未被 skip），只是把本账号排除掉，并且**带回归因
// 分布**——调用方要按当前白名单重判，`status = verified` 只代表"按当时的白名单判过"。
//
// **不按账号截断**：每账号只取一张的话，截断发生在重判之前——最晚过期的那张按当前白名单不合格、
// 更早的一张仍合格时，这个账号会被误判成接不了手。符合条件的票都是一小时内的，全量返回的量很小。
