...
// 流已开始后的票据拒服，经真实的 ops 中间件落库时也不得记成上游错误。
//
// 流一旦开始，拒服只能写成流内错误帧；中间件会从捕获的帧里解析出状态与文案。那是本地写的帧：
// 据它合成 upstream_status_code / upstream_error_message，就又把本地拒服记成了上游的锅，Messages
// 那条（`api_error`）还会按类型落成 502。
func TestKongStreamStartedTicketDenyOpsRecord(t *testing.T) {
	for _, anthropic := range []bool{false, true} {
		path := "/v1/responses"
		if anthropic {
			path = "/v1/messages"
		...
		t.Run(path, func(t *testing.T) {
			...
			router.POST(path, func(c *gin.Context) {
				...
				h := &OpenAIGatewayHandler{}
				if anthropic {
					h.handleAnthropicFailoverExhausted(c, kongDenyFailoverErr(service.KongDenyWindowClosed), true)
				} else {
					h.handleFailoverExhausted(c, kongDenyFailoverErr(service.KongDenyWindowClosed), true)
				}
			})
			router.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodPost, path, nil))

			if OpsErrorLogQueueLength() != 1 {
				t.Fatalf("应当记一条错误，队列里有 %d 条", OpsErrorLogQueueLength())
			}
			entry := (<-opsErrorLogQueue).entry
			if entry.StatusCode != http.StatusServiceUnavailable {
				t.Errorf("逻辑状态 = %d, want 503", entry.StatusCode)
			}
			if entry.UpstreamStatusCode != nil || entry.UpstreamErrorMessage != nil {
				t.Errorf("本地拒服不得带上游状态与文案：status=%v message=%v",
					entry.UpstreamStatusCode, entry.UpstreamErrorMessage)
			}
			if entry.ErrorPhase != "routing" || entry.ErrorOwner != "platform" || entry.ErrorSource != "gateway" {
				t.Errorf("归因 = %s/%s/%s, want routing/platform/gateway", entry.ErrorPhase, entry.ErrorOwner, entry.ErrorSource)
			}
		})
	}
}
