...
func kongLiveSidebandDenyClose(c *gin.Context, err error) (coderws.StatusCode, string, bool) {
	...
	service.MarkKongTicketDenyServed(c, reason)
	// WS 升级之后 HTTP 状态已经是 101，错误看板只认带内失败标记——不标就没有这条最终失败的记录。
	// 分类与原生 WS 的票据拒服一致。
	service.MarkOpsStreamFailure(c, "service_unavailable", "codex_ticket_unavailable:"+reason,
		"codex ticket unavailable on this transport [reason: "+reason+"]", http.StatusServiceUnavailable)
