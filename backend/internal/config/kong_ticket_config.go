...
// `gpt-6-astra`、`gpt-5.5` 或 `gpt-6-sol` 的票同样接受；请求 `gpt-6-sol` 时，归因为 `gpt-6-astra`
// 的票同样接受。
//
// **`gpt-6-astra`**（两个 sol 都收）：实测同一账号为 sol 取票时归因常返回 astra——拿到更高档位
// 不该被判成降智。方向是单向的，astra 不接受任何 sol，那正是这个功能要挡的东西。
//
// **`gpt-6-sol`**（只有 5.6-sol 收）：上游会把 gpt-6-sol 投到 5.6-sol 名下，拿到的是更新的模型，
// 与 stg0 白名单里的同名项理由相同（升级投放），不属于两张表互抄。校准资料含 gpt-6-sol 候选后，
// 5.6-sol 的真实回答也会分给它一部分概率（golden 样本里最高 0.23），不收就会把好票判成降智。
//
// **gpt-6-sol 不收 5.6-sol / 5.5**：那是跨代下降。按 ModelTrace 参考数据的交叉验证（2026-09-23
// 资料），gpt-6-sol 的回答单份能过 0.9 的只有 69%（不收 astra 时 58%），两份 92%、三份 100%——
// 单份不够时挑战循环会加份，代价是 gpt-6-sol 的验证平均更费额度；同一口径下 6-luna / 5.6-luna /
// 5.6-terra / 5.6-sol / 5.5 的回答没有一次被这组白名单放过。
//
...
const DefaultKongTicketAcceptExtra = "gpt-5.6-sol:gpt-6-astra,gpt-5.5,gpt-6-sol;gpt-6-sol:gpt-6-astra"
...
type KongCodexTicketConfig struct {
	...
	// 所以那一条由代码补齐、不交给配置。
	//
	// 为什么是白名单而不是一条档位链：校准资料里的候选跨 gpt 与 claude 两个家族，gpt-5.6 与
	// gpt-6 两代内部又各有几个同辈。它们之间并不存在一条全序，硬排成链等于凭空
